...
const faqs = [
  ['Is this actually free?', 'Yes, both the community and the August 14th conference are 100% free.'],
  ...
  ["What if I can't make it live?", 'Recordings are for community members, but only live attendees on August 14th get to ask questions—that only happens once.'],
  ['What time is this, and in what timezone?', 'August 14th, 8pm SGT (Singapore Time).'],
]
...
const stories = [
  {
    ...
    caption: 'It’s 6 pm, and you’re at dinner deciding what to eat, not what’s due tomorrow. The invoices, the scheduling, the fifteen small things eating your day: all handled without you touching them.', visual: 'visual-time', image: '/images/problem-1-workflow.png', reverse: false,
  },
  {
  ...
  },
  {
    ...
    caption: 'The systems that used to eat your whole week run themselves. You stop chasing bigger clients. They start finding you.', visual: 'visual-growth', image: '/images/problem-3-growth.png', reverse: false,
  },
]
...
export default function Home() {
  return <main>
    ...
    <section className="hero shell" id="top">
      ...
      <a className="video-card" href="#top" aria-label="Ainstein introduction video placeholder"><img src="/images/video-segment.png" alt="Ainstein community video call" /><div className="video-noise" /><span className="play-button"><Play size={20} fill="currentColor" /></span><span>Watch the Ainstein story</span><small>Video coming soon</small></a>
    </section>

    <section className="proof shell"><p>Trusted by the companies building the future</p><div className="partner-logos"><img src="/images/partners/animoca-brands.svg" alt="Animoca Brands" /><img src="/images/partners/uniswap.webp" alt="Uniswap" /><img src="/images/partners/og.png" alt="OG" /><img src="/images/partners/lovable-logo-cropped.png" alt="Lovable" /><img src="/images/partners/qwen-logo-cropped.png" alt="Qwen" /></div></section>

    <section className="why shell"><div className="why-lead"><span className="eyebrow"><Sparkles size={13} /> Built from the field</span><h2>Why Learn With <em>Ainstein?</em></h2></div><div className="why-copy"><p>We have interviewed 1,000+ founders, operators, and industry leaders; partnered with some of the world’s leading technology companies; and built Asia’s largest innovation communities.</p><p>Now we’re bringing that experience together to help anyone build a One-Person Company with AI.</p></div></section>

    <section className="founders shell"><div className="section-heading"><span className="eyebrow"><Sparkles size={13} /> The team</span><h2><em>Who’s</em> behind this</h2></div><div className="founder-grid"><article className="founder-card"><img src="/images/amos.png" alt="Amos, Ainstein founder" /><div><span>Founder</span><h3>Amos</h3><p>Built one of Southeast Asia’s fastest-growing new media companies.</p></div></article><article className="founder-card"><img src="/images/maeve-speaker.png" alt="Maeve speaking at an Ainstein event" /><div><span>Co-founder</span><h3>Maeve</h3><p>Generated over half a billion impressions across AI, web3 &amp; emerging technology.</p></div></article></div></section>

    <section className="struggles shell"><div className="section-heading"><span className="eyebrow"><Sparkles size={13} /> The founder bottleneck</span><h2>3 struggles <em>holding back</em> every founder.</h2></div><div className="story-list">{stories.map(story => <article className={`story ${story.reverse ? 'story--reverse' : ''}`} key={story.number}><div className={`story-visual ${story.visual} ${story.image ? 'story-visual--photo' : ''}`}>{story.image && <img src={story.image} alt="Founder working late at a laptop" />}<span>{story.number}</span><div className="visual-orb" /><p>{story.title}</p></div><div className="story-copy"><span className="story-kicker">THE PROBLEM / {story.number}</span><h3>{story.headline}</h3><ul>{story.bullets.map(bullet => <li key={bullet}>{bullet}</li>)}</ul><div className="story-caption"><span>THE SHIFT</span>{story.caption}</div>{story.number === '03' && <CommunityButton className="event-cta story-cta">Reserve your seat — free, limited to live attendees</CommunityButton>}</div></article>)}</div></section>

    <section className="event shell"><div className="event-lead"><span className="eyebrow"><Sparkles size={13} /> Live, one time only</span><h2>You don’t need another AI tutorial. You need <em>proven systems.</em></h2><p>That’s why we built Ainstein to give you front-row access to the workflows successful founders already use every day.</p><img className="event-workflow" src="/images/ai-content-workflow-white.png" alt="AI content-creation workflow connecting creation tasks to business outcomes" /></div><div className="event-detail"><p className="event-date">August 14th, 8pm SGT. One day. 5 hours. Live. Free.</p><p>We’re bringing 6 masters running real, high-pressure, high-value companies to show you exactly how they use AI to build a One-Person Company.</p><p>Laptops open. Real workflows. Real questions answered — live.</p><p>This isn’t a course, a podcast, or a replay. It’s one live session, one time only.</p><div className="scarcity"><span>LIVE ATTENDEES ONLY</span><p>Recordings are for watching only. Live attendees are the only ones who get to ask questions—and that only happens once, on August 14th.</p></div><p>What happens after August 14th? You’ll find out live, with everyone else.</p></div></section>

    <section className="community shell"><div><span className="eyebrow"><Sparkles size={13} /> One room, many perspectives</span><h2>Community around the <em>world.</em></h2><p>Founders, operators and builders learning together across Asia and beyond.</p></div><div className="flag-list" aria-label="Community locations"><span title="Malaysia">🇲🇾</span><span title="Singapore">🇸🇬</span><span title="Vietnam">🇻🇳</span><span title="Thailand">🇹🇭</span><span title="India">🇮🇳</span><span title="Indonesia">🇮🇩</span><span title="Korea">🇰🇷</span></div><div className="community-gallery"><img src="/images/community/community-1.png" alt="AInstein community gathering" /><img src="/images/community/community-2.png" alt="AInstein community session" /><img src="/images/community/community-3.png" alt="AInstein community event" /><img src="/images/community/community-4.png" alt="AInstein community members" /></div><div className="community-action"><CommunityButton>Reserve your seat — free, limited to live attendees</CommunityButton></div></section>
